            ...
            <div className="flex-1 p-6 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{provider.companyName}</span>
                                <div className="flex items-center text-amber-400 text-[10px] gap-0.5">
                                    <Star size={10} fill="currentColor" />
                                    <span className="text-zinc-500 font-medium">9.2</span>
                                </div>
                            </div>
                            <h3 className="text-xl font-black text-zinc-900 dark:text-white italic uppercase tracking-tighter leading-tight">
                                {car.description}
                            </h3>
                        </div>
                        <div className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide flex items-center gap-1">
                            <Shield size={10} />
                            Full Insurance
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 my-4">
                        ...
                        <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400 text-xs">
                            {/* Auto/Manual Icon */}
                            <span className="font-bold border border-zinc-200 dark:border-zinc-700 px-1.5 rounded-[4px] text-[9px] uppercase">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800 gap-4 sm:gap-0">
                    <div className="flex flex-col w-full sm:w-auto text-center sm:text-left">
                        <span className="text-zinc-400 text-[9px] uppercase font-black italic tracking-[0.2em] mb-1">Total Price</span>
                        <div className="flex items-baseline justify-center sm:justify-start gap-1">
                            <span className="text-3xl font-black italic text-[var(--brand-color)] tracking-tighter">{Math.ceil(price.total)}</span>
                            <span className="text-[10px] font-bold uppercase opacity-60">{price.currency}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        <button
                            ...
                            className="flex-1 sm:flex-none h-11 px-6 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 font-black italic uppercase text-[10px] tracking-[0.15em] border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all active:scale-95 flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                            onClick={() => onSelect(offer)}
                            className="flex-1 sm:flex-none h-11 px-6 rounded-xl text-white font-black italic uppercase text-[10px] tracking-[0.15em] shadow-lg hover:shadow-xl active:scale-95 transition-all relative overflow-hidden group/btn bg-[var(--brand-color)] flex items-center justify-center gap-2"
                        >
