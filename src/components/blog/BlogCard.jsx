...
export default function BlogCard({ post, index, currentUser, onDelete }) {
    const { t, language: lang } = useLanguage();
    const [showConfirm, setShowConfirm] = useState(false);

    // Check ownership:
    // If currentUser is passed (Public Blog Page), check if ids match.
    // If currentUser is NOT passed but onDelete is present (Profile Page logic might be different or assumed safe),
    // typically Profile passes onDelete only for owned posts.
    // Here we unify: If we have an onDelete function AND (no user check needed OR user owns it).

    // Simplification: In Blog.jsx we pass currentUser. In Profile.jsx we might render it differently.
    // Let's assume if currentUser is provided, we MUST check ownership.
    ...
    return (
        ...
        >
            <Card className="h-full flex flex-col rounded-[2.5rem] overflow-hidden border-none shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] bg-white dark:bg-[#0f0f0f] group hover:-translate-y-2 transition-all duration-500 relative">
                <div className="relative aspect-[4/3] overflow-hidden p-2">
                    <div className="w-full h-full rounded-[2rem] overflow-hidden relative">
                        ...
                        <div className="absolute top-4 left-4">
                            <Badge className="bg-[#3b60ff] hover:bg-[#3b60ff] text-white border-none px-4 py-2 font-black uppercase text-[10px] tracking-widest rounded-full shadow-lg">
                                {t("blog.storyTag")}
                            </Badge>
                        </div>
                        ...
                        <AnimatePresence>
                            {showConfirm && (
                                ...
                                >
                                    <p className="font-black uppercase italic tracking-widest text-[10px] mb-4 text-center leading-tight">
                ...
                <CardHeader className="pt-4 px-8 flex-grow space-y-4">
                    <div className="flex items-center gap-2 text-zinc-400 text-[10px] uppercase font-bold tracking-widest">
                        <Calendar size={12} />
                        {new Date(post.created_at).toLocaleDateString(lang === "de" ? "de-DE" : "en-US")}
                    </div>
                    <CardTitle className="text-3xl font-[1000] italic uppercase tracking-tighter text-zinc-900 dark:text-white leading-[0.9]">
                        {post.title}
                    </CardTitle>
                ...
                <CardFooter className="p-8 pt-0 mt-auto">
                    <Button
                        asChild
                        className="w-full h-14 bg-[#3b60ff] hover:bg-[#2f4ccc] text-white font-black uppercase tracking-widest rounded-2xl shadow-lg shadow-blue-500/20 group/btn border-none text-xs"
                    >
                        <Link
                            to={`/blog/${post.id}`}
